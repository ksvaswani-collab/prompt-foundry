...
export default function TokenList({ tokens, onChange }) {
    ...
    <div>
      {tokens.map((t, i) => (
        <div className="mb-2 flex gap-2" key={i}>
          ...
          />
          <button className="icon-btn" onClick={() => removeToken(i)}>×</button>
        </div>
      ))}
      <button className="add-link" onClick={addToken}>+ add token</button>
    </div>
  );
}
